{#
  Gold: текущее состояние каждого промокода за всю историю.
  Сколько раз использован (только completed), сколько осталось до лимита,
  использовался ли хоть раз после expiry. Семантика «здесь и сейчас» —
  каждый прогон полностью пересчитывает агрегат и апсёртит по promo_code_id.
  Метрика «доля транзакций с просроченным промо за день» лежит отдельно
  в promo_expired_usage_daily.
#}
{{ config(
