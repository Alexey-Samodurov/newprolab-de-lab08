{#
  Silver: очищенные транзакции — дедуп по composite_pk (последняя по
  ingested_at), нормализация типов и обогащение пользователем/промокодом.
  Транзакции с пустым/неизвестным user_id не выкидываем, а помечаем
  флагами — бизнес сам решает, считать ли по «чистым» пользователям.
  Здесь же раскладываем DQ-флаги (невалидная сумма, дубликат tx_id,
  просроченный промокод, тестовый юзер), которые потом крутят gold.
#}
{{ config(
