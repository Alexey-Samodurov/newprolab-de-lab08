...
def build_charts(client: SupersetClient, ds: dict[str, int | None]) -> dict[str, int | None]:
    ...
    charts["Daily Transactions by Status"] = upsert_chart(client, {
        ...
        "description": (
            "Дневной объём транзакций реальных пользователей: Completed (левая ось) "
        ...
        "params": json.dumps({
            "x_axis": "event_day",
            "time_grain_sqla": "P1D",
            "metrics": [simple_metric("completed_cnt", "SUM", "Completed")],
            "metrics_b": [simple_metric("failed_cnt", "SUM", "Failed")],
            "adhoc_filters": [REAL_USERS_FILTER],
            "adhoc_filters_b": [REAL_USERS_FILTER],
            "row_limit": 10000,
            "row_limit_b": 10000,
            "color_scheme": "supersetColors",
            "seriesType": "bar",
            ...
            "yAxisIndexB": 1,
            "y_axis_format": "~s",
            "y_axis_format_secondary": "~s",
            "y_axis_title": "Completed",
            "y_axis_title_secondary": "Failed",
            "y_axis_title_margin": 40,
            "yAxisTitleMargin": 40,
            "yAxisTitleMarginSecondary": 40,
            "y_axis_bounds": [0, None],
            "y_axis_bounds_secondary": [0, None],
            "truncateYAxis": False,
    ...
    charts["Transactions by Hour"] = upsert_chart(client, {
        ...
        "description": (
            "Распределение транзакций по часам суток: Real users vs Test users — "
        ...
        "params": json.dumps({
            "metrics": [
                sql_metric("SUM(CASE WHEN is_test_user = FALSE THEN tx_cnt ELSE 0 END)", "Real users"),
                sql_metric("SUM(CASE WHEN is_test_user = TRUE  THEN tx_cnt ELSE 0 END)", "Test users"),
            ],
            "groupby": ["hour_of_day"],
            "columns": [],
            "row_limit": 24,
